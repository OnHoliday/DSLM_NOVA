...
class NeuralNetwork(object):
    ...
    def load_sensors(self, X):
        ...
        self.bias.semantics = ones(sensor_data.shape[0])
        # self.bias.semantics = np.array([1 for _ in range(sensor_data.shape[0])])
    ...
    def add_sensors(self, X):
        ...
        self.sensors = [Sensor(np.array([])) for _ in X.T]
        self.load_sensors(X)
...
def create_output_neuron(activation_function, bias, initial_bias_connection_weight=0.0):
    neuron = Neuron(np.array([]), list(), activation_function, cache_weighted_input=True)
...
def get_final_output(x):
    ...
    arr = np.array([neuron.semantics for neuron in x])
...
class ConvNeuralNetwork(object):
    ...
    def calculate(self):
        ...
        """ TODO: To Konrad: fixed the flattening layer issue """
        for i in range(len(flatten_layer)):
            self.flatten_layer[i].semantics = flatten_layer[i, :]
        #=======================================================================
    ...
    def get_predictions(self):
        """Returns semantics of output neuron."""
        output = get_final_output(self.output_layer)
        # self.final_output = np.transpose(output)
        # return np.argmax(np.transpose(output), axis=1)
        """ TODO: To Konrad: added predictions for CE loss (self.y_pred_ce) and for accuracy (self.y_pred_labels) """
        self.y_pred_ce = np.transpose(output)
        self.y_pred_labels = np.argmax(self.y_pred_ce, axis=1)
        return self.y_pred_ce, self.y_pred_labels
    ...
    def load_semantic_after_flat_layer(self):
        ...
        for hidden_neuron in self.hidden_layers[0]:
            ...
            self.bias.semantics = np.array([1 for _ in range(flat_data.semantics.shape[0])])
    ...
    def predict(self, X):
        self.load_sensors(X)
        self.calculate()
        self.y_pred_ce, self.y_pred_labels = self.get_predictions()
        self.wipe_semantics()
        return self.y_pred_ce, self.y_pred_labels
    ...
    def wipe_semantics(self):
        ...
        self.bias.free_semantics()
         
        for sensor in self.sensors:
            sensor.free_semantics()
 
        for cl in self.cnn_layers:
            for neuron in cl:
                neuron.free_semantics()
         
        for neuron in self.flatten_layer:
            neuron.free_semantics()
