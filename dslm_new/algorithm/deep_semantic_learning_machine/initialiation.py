...
def init_conv_standard(X, y, input_layer, random_state, learning_step_function=init_ols_balanced,
                       init_conv_min_layers=1, init_conv_max_layers=3,
                       init_min_conv_neurons_per_layer=1, init_max_conv_neurons_per_layer=5, prob_conv=0.75,
                       init_min_layers=1, init_max_layers=1, init_min_neurons_per_layer=1, init_max_neurons_per_layer=10,
                       init_last_hl_neurons=100, max_neuron_connection_weight=0.1, max_bias_weight=0.1,
                       hidden_activation_functions_ids='relu', prob_activation_hls=1,
    ...
    number_of_cls = random_state.randint(init_conv_min_layers, init_conv_max_layers + 1)

    number_conv_neurons = [random_state.randint(init_min_conv_neurons_per_layer, init_max_conv_neurons_per_layer + 1)
                             for _ in range(number_of_cls)]

    number_of_hls = random_state.randint(init_min_layers, init_max_layers + 1)

    number_hidden_neurons = [random_state.randint(init_min_neurons_per_layer, init_max_neurons_per_layer + 1)
                             for _ in range(number_of_hls)]
    ...
    sparseness = {'sparse': False, 'minimum_sparseness': .25, 'maximum_sparseness': .75,
                  'prob_skip_connection': 0}
    nn = ConvNeuralNetworkBuilder.generate_new_conv_neural_network(
                                                            number_conv_neurons,
                                                            number_of_cls,
                                                            prob_conv,
                                                            number_of_hls,
                                                            ...
                                                            y.shape[1],
                                                            y.shape[0],
                                                          max_neuron_connection_weight, max_bias_weight,
                                                          output_activation_function, input_layer, random_state,
