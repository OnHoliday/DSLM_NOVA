...
class Connection:
    ...
    def _is_from_previous_neuron(self):
        ...
        if isinstance(self.from_neuron, ConvNeuron) or isinstance(self.from_neuron, PoolNeuron):
            return True

        if isinstance(self.from_neuron, FlatNeuron):
            return True

        if isinstance(self.from_neuron, FlatNeuron) \
                and isinstance(self.to_neuron, HiddenNeuron) \
                and self.to_neuron.level_layer == 0:
            return True
